...
void GPIO_setPortDirection_H_L(uint8 Port, uint8 Position, uint8 Mode) {
	...
	case GPIOA_ID:
		...
		if (Position == LOW_PINS) {
			GPIOA->CRL = 0x00000000;
			for (uint8 i = 0; i < 8; i++) {
				GPIOA->CRL |= Mode << (i * 4);
			}
		} else if (Position == HIGH_PINS) {
			GPIOA->CRH = 0x00000000;
...
void GPIO_setPortValue_H_L( uint8 Port , uint8 Position , uint16 Value ){
	...
	case GPIOA_ID:
		...
		if      ( Position == LOW_PINS  ){ GPIOA->ODR = (GPIOA->ODR & 0xFF00 ) | ( (uint8)Value ) ; }
		else if ( Position == HIGH_PINS ){ GPIOA->ODR = (GPIOA->ODR & 0x00FF ) | (    ((uint8)Value) << 8 ) ; } /* added by me */

		break;
	case GPIOB_ID:

		if      ( Position == LOW_PINS  ){ GPIOB->ODR = (GPIOB->ODR & 0xFF00 ) | ( (uint8)Value ) ; }
		else if ( Position == HIGH_PINS ){ GPIOB->ODR = (GPIOB->ODR & 0x00FF ) | (     Value ) ; }

		break;
	case GPIOC_ID:

		if      ( Position == LOW_PINS  ){ GPIOC->ODR = (GPIOC->ODR & 0xFF00 ) | ( (uint8)Value ) ; }
		else if ( Position == HIGH_PINS ){ GPIOC->ODR = (GPIOC->ODR & 0x00FF ) | (     Value ) ; }
